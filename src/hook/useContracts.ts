...
const useContracts = (): any => {
  ...
  const getContractConfig = (): any => {
    ...
    const nft_contract = new ethers.Contract(CONTRACT_ADDRESS, contractABI, getSigner);
    const market_contract = new ethers.Contract(Market_ADDRESS, contractMarketABI, getSigner);
  ...
  const readCollabPercent = async (tokenId: string) => {
    ...
    if (contract !== undefined) {
      ...
      try {
        const getContract = getContractConfig();
        const res = await getContract.nft_contract.collaboratotPercentageOf(INTtokenID);
        let CollabPercent = 0;
        res.forEach((value: BigNumber) => {
          CollabPercent += value.toNumber()
        });
  ...
  const sellNFT = async (tokenId: string, price: number, idDocNFT: string) => {
    ...
    if (contract_market !== undefined && signer !== undefined) {
      try {
        ...
        setWaitTransaction(true);
        await tx.wait();
        await axios.patch(`${baseUrl}/nft/listingForSale?ownerAddress=${address}`, {
          id: idDocNFT,
        },
          config);
        return true;
  ...
  const cancelSellNFT = async (tokenId: string, idDocNFT: string) => {
    if (contract_market !== undefined && signer !== undefined) {
      try {
        ...
        setWaitTransaction(true);
        await tx.wait();
        await axios.patch(`${baseUrl}/nft/unlistingForSale?ownerAddress=${address}`, {
          id: idDocNFT,
        },
          config);
        return true;
  ...
  const getPrice = async (tokenId: string) => {
    if (contract_market !== undefined) {
      ...
      try {
        const getContract = getContractConfig();
        const price = await getContract.market_contract.priceFromTokenId(tokenId);
        return price;
      } catch (e) {
        return 0;
      }
